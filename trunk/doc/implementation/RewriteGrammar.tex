...
\end{figure}
Explained with an example input character stream, \verb!<a id="1"> b </a>!, and where \emph{X} marks the spot where the lexer is in the respective state, our five defined states are:
\begin{description}
\item[DEFAULT] X %\verb!<a id="1"> b </a>!
%\item[IN_TAG] \texttt{<a \textnormal{\emph{X}} id="1"> b </a>}
\item[IN_APOS_ATTRIBUTE] text
\item[IN_QUOT_ATTRIBUTE]text
\item[IN_ELEMENT]text
\end{description}
.
