...
XQuery Full Text\cite{w3c01} is an superset of XQuery. A quick overview
...
Where when the comparison operator \texttt{COMP} is \texttt{<<} the MQL function \textsf{isFUNK} must be
\textsf{isPreceding}, and when the operator is \texttt{>>} the function must be \textsf{isFollowing}.
...
anti-join operator, the same effect may be achieved by using a lef-outer-join followed by a selection. A
