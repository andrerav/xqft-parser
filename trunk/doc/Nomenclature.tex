...
\nomenclature{\textbf{Predicate, syntactic}}{\emph{ANTLR v3 specific}. A
selective form of backtracking used to recognise language constructs that cannot be distinguished with finite look-ahead. See section \ref{sect:antlr:syntacticPredicate}.}
...
\nomenclature{\textbf{Terminal production}}{A production that cannot be further divided (in the traditional sense. For ANTLR, see \textbf{fragment}) and generates a token. Also called lexer production. See section \ref{sect:antlr:lexer}}
...
\nomenclature{\textbf{Non-terminal production}}{A production that consists of references to other non-terminals or terminals. Also called parser production. See section \ref{sect:antlr:parser}.}
...
\nomenclature{\textbf{Non-greedy subrule}}{A referenced rule that will only match input until the referring rule has another alternative that can match the same input. See \textbf{greedy subrule}.}
...
\nomenclature{\textbf{Non-determinism}}{ See \textbf{Ambiguous}. }
...
\nomenclature{\textbf{Recogniser}}{A general term for a program with the task
of recognising patterns. E.g. a parser or a lexer.}
