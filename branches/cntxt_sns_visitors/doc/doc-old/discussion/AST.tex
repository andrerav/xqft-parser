...
\ref{sect:method:alternatives}). With this tool the AST construction has to be done
manually from scratch by adding action code to the grammar. However, this ease of change may make it easy to break implicit
...
flow analysis. Thus it is suited for implementation of type checking,
...
``\verb!with|without stemming!") to nodes. As seen in figure \ref{tree:ast:ftq1}, we have simply
...
grammar. It would be benefitial to have this logic removed and decoupled. This would be complementeded with a data flow analysis framework with the necessary facilities for
