...
The \textsf{project()} operator lends its name and semantics from the project
(section \ref{sect:theory:relAlg:projection}) and the rename operator (section \ref{sect:theory:relAlg:rename}) operator
known from basic relational algebra. The \textsf{project()} operator has a more
complex syntax described in EBNF notation in figure \ref{figure:mql:ebnf:project_ebnf}.
