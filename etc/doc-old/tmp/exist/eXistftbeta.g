 ...
 *  
 *  $Id: XQuery.g 6214 2007-07-11 13:04:56Z itakhil $
 */
header {
...
:
	decl:"declare"! ftopt:"ft-option"! ftMatchOption
	;
...
{ String varName; }
:
	"score"^ DOLLAR! varName=qName
	{ #ftScoreVar= #[FTSCORE_VAR, varName]; }
	;

letVarBinding throws XPathException
{ String varName; }
:
	DOLLAR! varName=qName! ( typeDeclaration )? ( ftScoreVar )?
	COLON! EQ! exprSingle
...
:
	ftOr ( ( ftMatchOption | ftProximity )* ( "weight"^ rangeExpr )? )
	;
...
:
	ftUnaryNot ( "mild"^ "not"^ ftUnaryNot )*
	;

ftUnaryNot throws XPathException
:
	(BANG^)? ftWordSelection
	;

ftWordSelection throws XPathException
:
	( ftWords ( ftTimes )? )
...
ftProximity throws XPathException
:
	ftOrderedIndicator | ftWindow | ftDistance | ftScope | ftContent
	;
	
ftOrderedIndicator throws XPathException
:
	"ordered"^
	;
...
:
	...
	| ftDiacriticsOption
	| ftLanguageOption
	| (  "with"^ ( ftOnlyDiacriticsOption  | ftStemOption | ftWildCardOption | ftWithThesaurusOption | ftWithStopWordOption )
		| "without"^ ( ftOnlyDiacriticsOption | ftStemOption | ftWildCardOption | ( "thesaurus"^ ) | ( "stop"^ "words"^ ) )
		)
	;

ftCaseOption throws XPathException
:
	"lowercase"^
	| "uppercase"^
...
ftOnlyDiacriticsOption throws XPathException
:
	"diacritics"^
	;
...
ftStemOption throws XPathException
:
	( "stemming" )
	;

ftWithThesaurusOption throws XPathException
:
	( "thesaurus"^ ( ftThesaurusID | "default" ) )
	| ( "thesaurus"^ LPAREN! ( ftThesaurusID | "default" ) ( COMMA! ftThesaurusID )* RPAREN! )
	;
...
ftWithStopWordOption throws XPathException
:
	( ( "stop"^ "words"^ ftRefOrList ) | ( "default"^ "stop"^ "words"^ ) ) 
	( ftInclExclStringLiteral )* 
	;

ftRefOrList throws XPathException
:
	( "at"^ STRING_LITERAL^ )
	| ( LPAREN! STRING_LITERAL^ ( COMMA! STRING_LITERAL^ )* RPAREN! )
	;

ftInclExclStringLiteral throws XPathException
:
...
ftWildCardOption throws XPathException
:
	( "wildcards"^ )
	;

ftContent throws XPathException
:
	( "at"^ ( "start"^ | "end"^ ) ) 
	| ( "entire"^ "content"^ )
	;
...
ftScope throws XPathException
:
	( "same"^ | "different"^ ) ftBigUnit
	;

ftUnit throws XPathException
:
	"words"^ | "sentences"^ | "paragraphs"^
	;

ftBigUnit throws XPathException
:
	"sentence"^ | "paragraph"^
	;
...
protected BANG options { paraphrase="!"; }: "!" ;
